...
- **Social cards (og:image)** — every page's card is a small 1200×630 image (never a raw multi-MB source); `ogCard` filter + `scripts/gen-og-cards.mjs` (cards for portraits, writing covers, SVG artifacts) + a build lint (`scripts/check-og-images.mjs`) — in `.claude/rules/og-images.md`.
