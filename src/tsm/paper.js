...
//
// Vendored verbatim from linxule.com (src/assets/js/paper.js) so the demo
// generates the SAME living paper as the rest of the site: a unique seamless
// tile per session from stock parameters + a session seed. Self-contained
// (simplex noise + RNG inlined, no imports), so it stays subpath-safe — no
// dependency on /assets being present at the site root.
//
// ONE adaptation for the demo (see detectStock): the demo is a single working
// artifact, so the stock is pinned to "workshop" (the design system's stock
// for working pages — "material has presence") instead of being inferred from
// the path. That keeps the texture identical whether served locally at "/" or
// deployed at "/tsm". Everything else is the site's generator unchanged.
...
(function() {
    ...
    // ── Stock Selection ────────────────────────────────────────────
    // The demo is a single working artifact (build-the-matrix tool), so it
    // pins to the "workshop" stock — the site's stock for working pages where
    // the material has presence. Pinning (rather than inferring from the path)
    // keeps the paper identical whether served at "/" locally or "/tsm" live.
    ...
    // The flat --paper color + SVG-noise fallback show first (CSS), then the
    // generated tile replaces them.
